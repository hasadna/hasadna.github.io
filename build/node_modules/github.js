...
function getFolderName(str) {
    ...
    return str.substring(lastSlash + 1, str.length - 4);
}
...
var cloneOrPull = function(repoURL, callBack, verbose) {
    ...
    var gitFolder = '--git-dir=' + parentPath + '/' + containingFolder + '/.git';

    if (path.existsSync(parentPath + '/' + containingFolder)) // If repository exist pull it, otherwise clone it.
...
var getLogForFile = function(fullPath, callBack) {
    ...
    var args = new Array(
            '--git-dir=' + dotGitPath,
            "log",
            "--pretty=format:%H" + separator + "%an" + separator + "%ae" + separator + "%ad" + separator + "%f" + separator + "%b",
            "--",
    ...
    process.on('exit', function(code) { // When exit run the callback with the results.
        ...
        for (var i = 0; i < resArr.length - 1; i++)// Sanitize everything
            resArr[i] = sanitizeJSON(resArr[i]);
