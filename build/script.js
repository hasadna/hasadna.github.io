...
svg.append("linearGradient")
        ...
        .attr("x1", 0).attr("y1", 0)
        .attr("x2", 0).attr("y2", 600)
        .selectAll("stop")
        .data([
            {offset: "0%", color: "steelblue"},
            {offset: "100%", color: "#153a5a"}
...
var httpRequest = $.ajax({
    url: "https://api.github.com/repos/hasadna/Open-Knesset/commits?top=master"
});
...
//            window.console.log(JSON.stringify(data));
    var loadData = function(i) {
        if (i === -1) {
            window.console.log(JSON.stringify(commArr));
            return;
        }
        var request = $.ajax({
            url: data[i].url
